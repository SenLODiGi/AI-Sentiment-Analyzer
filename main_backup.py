...
# Install: pip install nicegui textblob
...
# ---------- CONFIG ----------
PROFILE_IMAGE = 'image.png'  # <-- place your image file here (image.png) or change path
...
# ---------- STATE ----------
analysis_history = []  # list of dicts: {timestamp, text, sentiment, polarity, subjectivity}
...
def analyze_text_blob(text: str):
    ...
    result = {
        ...
        "key_phrases": list(dict.fromkeys(key_phrases))[:10],  # preserve order unique
    }
    return result
...
def read_profile_image_datauri(path):
    try:
        ...
        # guess simple mime by extension
        ext = path.split('.')[-1].lower()
        mime = "image/png" if ext in ("png","apng") else f"image/{ext}"
        return f"data:{mime};base64,{b}"
    except Exception:
        # return a neutral SVG datauri avatar
        svg = """
...
# Head CSS + Bootstrap Icons + animations for modern responsive look
ui.add_head_html('''
...
with ui.column().classes('main-container'):
    # Header
    with ui.card().classes('header-glass'):
        ui.html('<div style="font-size:40px;margin-bottom:10px;">🧠</div>')
        ui.label("AI Sentiment Analyzer").classes('text-3xl').style('font-weight:800;color:white;margin-bottom:8px;')
        ui.label("Analyze text sentiment with advanced AI insights").classes('text-sm').style('color:rgba(255,255,255,0.7);')

    # Main content grid
    with ui.row().classes('content-grid').style('width:100%;'):
        # left column: input + results (wider)
        with ui.column().classes('flex-1').style('min-width:0;'):
            with ui.card().classes('card-glass'):
                ...
                with ui.row().classes('items-center gap-3').style('margin-top:12px;'):
                    def do_analyze():
                    ...
                    def do_deep_analyze():
                        ...
                        ui.timer(1.0, lambda: show_deep_analysis_result(text), once=True)
                    
                    def show_deep_analysis_result(text):
                    ...
                    with result_box:
                        ...
                        # Sentence analysis
                        with ui.card().classes('glass-strong').style('padding:18px;margin-bottom:15px;'):
                            ui.label("🔍 Sentence-by-Sentence Analysis").classes('text-lg').style('font-weight:700;color:white;margin-bottom:10px;')
                            with ui.column().classes('gap-2').style('max-height:300px;overflow:auto;'):
                                for sentence in deep_analysis["sentences"][:5]:  # Show first 5 sentences
    ...
    # right sidebar: profile + quick examples + history
    with ui.column().style('min-width:320px;'):
        ...
        # Quick examples
        with ui.card().classes('card-glass').style('margin-bottom:16px;'):
            ui.label("⚡ Quick Examples").classes('text-base').style('font-weight:700;color:white;')
            with ui.column().classes('gap-2').style('margin-top:8px;'):
                def set_example(v): 
                    text_input.value = v
                ui.button("😊 Positive example", on_click=lambda:set_example("I absolutely love this product! The quality is outstanding and delivery was super fast. Highly recommend to everyone!")).classes('link-btn').style('width:100%;margin-bottom:4px;font-size:12px;padding:6px 12px;')
                ui.button("😔 Negative example", on_click=lambda:set_example("Very disappointed with this purchase. The item arrived damaged and customer support was unhelpful and rude.")).classes('link-btn').style('width:100%;margin-bottom:4px;font-size:12px;padding:6px 12px;')
                ui.button("😐 Neutral example", on_click=lambda:set_example("The meeting is scheduled for Monday at 10 AM in conference room B. Please confirm your attendance by Friday.")).classes('link-btn').style('width:100%;margin-bottom:4px;font-size:12px;padding:6px 12px;')
                ui.button("📱 Social Media", on_click=lambda:set_example("Just watched an amazing movie! The cinematography was breathtaking but the plot felt a bit rushed. Overall worth watching! #MovieNight")).classes('link-btn').style('width:100%;margin-bottom:4px;font-size:12px;padding:6px 12px;')
...
# ensure history is rendered once
refresh_history_list()
